...
img_formats = ['bmp', 'jpg', 'jpeg', 'png', 'tif', 'tiff', 'dng', 'webp', 'exr']  # acceptable image suffixes
...
def num_channles(layers):
    ...
    return sum([nc[chan] for chan in layers])
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32, layers=[L_RGB]):
        ...
        for layer in self.used_layers:
            self.layer_files[layer] = sorted([x.replace('/', os.sep) for x in files if x.split('.')[-1].lower() in img_formats and layer.lower() in x.lower()])
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for layer in self.used_layers:
                self.layer_files[layer] = sorted([x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in img_formats and layer.lower() in x.lower()])
            self.img_files = self.layer_files[self.used_layers[0]]
        ...
        # Check cache
        self.label_files = img2label_paths(self.img_files, self.used_layers[0])  # labels
        ...
        self.label_files = img2label_paths(cache.keys(), self.used_layers[0])  # update
        if single_cls:
        ...
        if self.rect:
            ...
            for layer in self.layer_files:
                self.layer_files[layer] = [self.layer_files[layer][i] for i in irect]
        ...
        if cache_images:
            ...
            results = ThreadPool(8).imap(lambda x: load_data(*x), zip(repeat(self), range(n)))  # 8 threads
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < hyp['mixup']:
                layers2, labels2 = load_mosaic(self, random.randint(0, self.n - 1))
                r = np.random.beta(8.0, 8.0)  # mixup ratio, alpha=beta=8.0
                layers = {k: (layers[k] * r + layers2[k] * (1 - r)).astype(np.uint8) for k in layers}
            ...
            for k in layers:
                layers[k], ratio, pad = letterbox(layers[k], shape, auto=False, scaleup=self.augment)
        ...
        if self.augment:
            # Augment imagespace
            if not mosaic:
                layers, labels = random_perspective(layers, labels,
                                                 degrees=hyp['degrees'],
            ...
            # Augment colorspace
            if L_RGB in layers:
                augment_hsv(layers[L_RGB], hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
        ...
        if nL:
            ...
            labels[:, [2, 4]] /= next(iter(layers.values())).shape[0]  # normalized height 0-1
            labels[:, [1, 3]] /= next(iter(layers.values())).shape[1]  # normalized width 0-1

        if self.augment:
            # flip up-down
            if random.random() < hyp['flipud']:
                layers = {k: np.flipud(layers[k]) for k in layers}
                if nL:
                    labels[:, 2] = 1 - labels[:, 2]

            # flip left-right
            if random.random() < hyp['fliplr']:
                layers = {k: np.fliplr(layers[k]) for k in layers}
                if nL:
                    labels[:, 1] = 1 - labels[:, 1]
    ...
    def collate_fn4(batch):
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                ...
                ly = torch.cat((torch.cat((layer[i], layer[i + 1]), 1), torch.cat((layer[i + 2], layer[i + 3]), 1)), 2)
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        # place img in img4
        if i == 0:  # top left
            layers4 = {k: np.full((s * 2, s * 2, layers[k].shape[2]), 114, dtype=np.uint8) for k in layers}  # base image with 4 tiles
    ...
    # Augment
    layers4, labels4 = random_perspective(layers4, labels4, segments4,
...
def load_mosaic9(self, index):
    """Cretes a mosaic out of 9 images, it will apply transforms on the final image if this is a training run.

    Args:
        index (int): The original image

    Returns:
        tuple: Updated Images and Labels
    """
    ...
    for i, index in enumerate(indices):
        ...
        # place img in img9
        if i == 0:  # center
            layers9 = {k: np.full((s * 3, s * 3, layers[k].shape[2]), 114, dtype=np.uint8) for k in layers} # base image with 9 tiles
            h0, w0 = h, w
        ...
        # Image
        for k in layers:
            layers9[k][y1:y2, x1:x2] = layers[k][y1 - pady:, x1 - padx:] # img9[ymin:ymax, xmin:xmax]
        hp, wp = h, w  # height, width previous

    # Offset
    yc, xc = [int(random.uniform(0, s)) for _ in self.mosaic_border]  # mosaic center x, y
    layers9 = {k: layers9[k][yc:yc + 2 * s, xc:xc + 2 * s] for k in layers}
    ...
    # Augment
    layers9, labels9 = random_perspective(layers9, labels9, segments9,
                                       degrees=self.hyp['degrees'],
                                       translate=self.hyp['translate'],
                                       scale=self.hyp['scale'],
                                       shear=self.hyp['shear'],
...
def random_perspective(layers, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
                       border=(0, 0)):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]

    height = next(iter(layers.values())).shape[0] + border[0] * 2  # shape(h,w,c)
    width = next(iter(layers.values())).shape[1] + border[1] * 2

    # Center
    C = np.eye(3)
    C[0, 2] = -next(iter(layers.values())).shape[1] / 2  # x translation (pixels)
    C[1, 2] = -next(iter(layers.values())).shape[0] / 2  # y translation (pixels)
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            layers = {k: cv2.warpPerspective(layers[k], M, dsize=(width, height), borderValue=(114, 114, 114)) for k in layers}
        else:  # affine
            layers = {k: cv2.warpAffine(layers[k], M[:2], dsize=(width, height), borderValue=(114, 114, 114)) for k in layers}
